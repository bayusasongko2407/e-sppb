...
return [
    ...
    'info' => [
         ...
         */
        'description' => <<<'MD'
## E-SPPB Enterprise API v1

API untuk integrasi sistem pengelolaan **Surat Permintaan Pengeluaran Barang (SPPB)** dan **Surat Jalan**.

**Base URL:** `https://e-sppb.engiboard.web.id/api/v1`

### Autentikasi
Gunakan **Bearer Token** (Laravel Sanctum). Token diperoleh dari endpoint `POST /auth/login` dan berlaku selama **8 jam**.
...
### Alur Utama
1. Login → dapatkan `access_token`
2. Buat SPPB (`POST /sppb`)
3. Submit untuk persetujuan (`POST /sppb/{uuid}/submit`)
4. Approver menyetujui via Workflow (`POST /workflow/steps/{stepId}/approve`)
5. Buat Surat Jalan (`POST /sppb/{uuid}/goods-releases`)
6. Konfirmasi penerimaan (`POST /goods-releases/{uuid}/receive`)
MD,
    ],

    'ui' => [
        'title' => 'E-SPPB Enterprise API',
    ],
    ...
    'middleware' => [
        'web',
        // Docs are public so AI Studio and frontend devs can access them
